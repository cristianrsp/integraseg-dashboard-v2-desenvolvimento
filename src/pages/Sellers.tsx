...
import { Plus, Trash2, Users, Calendar, Loader2 } from 'lucide-react';
    ...
    <MainLayout>
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Gerencie a equipe de vendas</p>
        </div>
          ...
          <DialogContent>
            <form onSubmit={handleSubmit}>
              <DialogHeader>
                ...
                <DialogDescription>Preencha o nome do vendedor.</DialogDescription>
              </DialogHeader>
              <div className="grid gap-4 py-4">
                <div className="grid gap-2">
                  <Label htmlFor="name">Nome Completo</Label>
                  <Input id="name" value={name} onChange={(e) => setName(e.target.value)} placeholder="Ex: João da Silva" />
                </div>
              </div>
              <DialogFooter>
      ...
      {sellers.length > 0 ? (
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>
                            Tem certeza que deseja remover o vendedor "{seller.name}"? Esta ação não pode ser desfeita.
                          </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
                          <AlertDialogCancel>Cancelar</AlertDialogCancel>
                          <AlertDialogAction onClick={() => handleDelete(seller.id, seller.name)}
        ...
        <div className="rounded-lg border border-dashed border-border bg-muted/30 p-12 text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">Clique no botão acima para cadastrar.</p>
        </div>
      )}
    </MainLayout>
  );
